...
export function Navbar() {
  ...
  const navItems = [
    ...
    { label: "Upcoming Event", href: "/upcoming-events" },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center justify-center flex-1">
            <div className="flex items-center bg-white rounded-full border border-gray-200 shadow-sm px-1.5 py-1.5 overflow-x-auto scrollbar-hide" style={{ maxWidth: "fit-content" }}>
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`px-3 xl:px-4 py-2 text-xs xl:text-sm font-medium rounded-full transition-all duration-200 whitespace-nowrap ${
                    isActive(item.href)
          ...
          {/* Right side buttons */}
          <div className="hidden lg:flex items-center gap-2 xl:gap-3 flex-shrink-0 ml-4">
            {/* Admin Button */}
            <Link href="/admin/login">
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Button className="text-xs xl:text-sm px-3 xl:px-5 bg-slate-700 hover:bg-slate-800 text-white rounded-full font-medium shadow-md hover:shadow-lg transition-all whitespace-nowrap">
                  Admin
                </Button>
              </motion.div>
            </Link>
            ...
            {!isLoading && (
              <>
                {user ? (
                  <Link href="/dashboard">
                    <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                      <Button className="text-xs xl:text-sm px-3 xl:px-6 bg-gradient-to-r from-amber-400 to-orange-500 hover:from-amber-500 hover:to-orange-600 text-white rounded-full font-semibold shadow-md hover:shadow-lg transition-all whitespace-nowrap">
                        Dashboard
                      </Button>
                    </motion.div>
                  </Link>
                  ...
                  <>
                    <Link href="/login">
                      <span className="text-xs xl:text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors cursor-pointer whitespace-nowrap">
                        Sign In
                      </span>
                    </Link>
                    <Link href="/signup">
